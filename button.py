...
class Button:
    def __init__(self, screen, x: int, y: int, width: int, height: int, color='black', text: str = None,
    ...
    def create_ellipse(self, outline=None):
        if not self.only_text:
            if outline:
                pygame.draw.ellipse(self.screen, outline, (self.x - 2, self.y - 2, self.width + 4, self.height + 4))
            if self.image:
                self.screen.blit(self.image, self.image.get_rect(x=self.x, y=self.y))
            else:
                pygame.draw.ellipse(self.screen, self.color, (self.x, self.y, self.width, self.height), 0)
